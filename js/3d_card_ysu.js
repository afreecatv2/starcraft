let calculateAngle = function(e, item, parent) {
		...
		if(parent.classList.contains('offline')){	
			calcAngleX = 0 - calcAngleX;
		}
		...
		item.style.transform = `rotateY(${calcAngleX}deg) rotateX(${-calcAngleY}deg) scale(1.1)`;
        parent.querySelector('.inner-card-backface').style.transform = `rotateY(${calcAngleX}deg) rotateX(${-calcAngleY}deg) scale(1.1) translateZ(-14px)`;
    ...
    document.querySelectorAll('.card').forEach(function(item) {
		...
		var icons = item.querySelectorAll('.icon-flip');
		if(icons) {
