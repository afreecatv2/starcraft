var radios = document.querySelectorAll('input[type=radio][name=prize]');
const gacha_type = localStorage.getItem('gacha_type');
if(gacha_type){
	radios[parseInt(gacha_type)].checked = true;
	if(gacha_type == 0){
		if(localStorage.getItem('gacha_type')){
			document.getElementById('inputPrize1').value = localStorage.getItem('gacha_number');
		}else{
			localStorage.getItem('gacha_type').value = 1;
		}
	}else{
		document.getElementById('ta_custom').value = localStorage.getItem('gacha_names');
	}
}
...
const getPrize = async () => {
	const prizes = [
		{
			image: 'https://starcraft.aengji.com/img/bj_high/깅예솔.jpg',
			title: 'Bunny'
		},
		{
			image: 'https://starcraft.aengji.com/img/bj_high/깅예솔.jpg',
...
const prepare = () => {
	...
	tl.to($machine, {
      ...
      $handle.style.cursor = 'pointer';
			...
			$handle.addEventListener('click', start);
...
const start = async () => {
	...
	if(once){
		...
		if(radios[0].checked){
			localStorage.setItem('gacha_type', 0);
			 localStorage.setItem('gacha_number', document.getElementById('inputPrize1').value);
			var number = (Math.floor(Math.random() * document.getElementById('inputPrize1').value)) + 1;
			document.querySelector('.prize-container .prize span').textContent = number;
			prize = {
				title: number
			};
			
		}else{
			if(document.getElementById('ta_custom').value.trim() == ''){
				alert("뽑기 내용을 입력하세요.");
				return;
			}
			
			var names = document.getElementById('ta_custom').value.split('\n');
			names = names.filter(item => item);
			
			localStorage.setItem('gacha_type', 1);
			localStorage.setItem('gacha_names', document.getElementById('ta_custom').value);
			
			var number = (Math.floor(Math.random() * names.length));			
			document.querySelector('.prize-container .prize span').textContent = names[number];
			prize = {
				title: names[number]
			};
			if(bj_names.includes(names[number])){
				...
				var match = false;
				for(var bj of bj_names){
					if(bj.includes(names[number])){
						names[number] = bj;
						match = true;
						break;
					}
...
const pop = () => {
  confetti($app, {
	...
	$title.children[0].innerHTML = `뽑기 결과는..<br>${prize.title}`;
	gsap.to($title, {
		delay: 1,
		y: '5vh',
		duration: 0.6
