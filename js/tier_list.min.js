...
const url = 'http://aengji.dothome.co.kr/afreecatv/select_sc_tier.php?name=' + name;
			...
			if(bj_tier2[key].length > item_cnt){
				...
				for(var r=0;r<row_cnt-1;r++){
					...
					for (var j = (r*item_limit); j < item_idx ; j++) {
						...
						var img = ce(ti, 'img', {'data-src': bjImgUrl(bj_name),'class': 'lazy star-tier-image image_' + bj_tier2[key][j][1],'onerror':'this.onerror=null; this.src="img/bj/noname.png"'});
						...
						if (match) {
							...
							onairHref = 'https:///bj.afreecatv.com/' + bj_tier2[key][j][2];
							img.setAttribute('onairhref', onairHref);
							img.addEventListener('dblclick', function (e) {
								window.open(e.target.getAttribute('onairhref'), '_blank');
							});
